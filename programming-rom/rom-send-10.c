...
int main(int argc, char argv[])
{
	...
	switch(buff[0]) {
	case 'r':
	  target = reset(argv[1], target, fd, chip);
	  break;
	case 'p':
	  sendpage(target, fd);
	  break;
...
FILE * reset(char location[], FILE *old, int fd, int chip)
{
  FILE *target;
  fclose(old);
...
void sendpage(FILE *target, int fd)
{
  char c;
